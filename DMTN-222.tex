...
\setDocDOI{10.71929/rubin/2586498}
...
\setDocChangeRecord{%
  ...
  \addtohist{8}{2026-07-22}{Add calibtool utility descriptions.}{Chris Waters}
}
...
\subsection{LSSTCam Flats}

At the present time, we have two ways of generating ``white light'' dome flats with LSSTCam data.
In the future, we will be adding monochromatic flats from scans with the tunable laser.

The term ``white light'' refers to illumination with the calibration system LED projector, with broad (``white'') and not narrow (laser) illumination.
The light itself is not white, and there are 10 LEDs covering the wavelength range of the ugrizy filter system.

The original configuration (also used for the daily flats) can illuminate each of the griz filters with 2 LEDs at a time.
These can be processed with the \texttt{cpFlat.yaml} pipeline and the \texttt{bps\_flat\_[band].yaml} templates.

The new configuration produces two sets of flats for each filter, one with a bluer LED and one with a redder LED (each appropriate for the filter).
These red and blue flats then get individually processed and weighted and combined at the end.
We call these ``anaglyph'' flats, in an analogy with the anaglyph stereoscopic photographic technique where two red/blue images are overlaid and viewed with special red/blue glasses.
These can be processed with the \texttt{cpFlatAnaglyph.yaml} pipeline and the \texttt{bps\_flat\_anaglyph\_[band].yaml} templates.
...
\begin{verbatim}
$CP_VERIFY_DIR/bin/cpv_report.py -r embargo \
...
First, calibrations in \texttt{main} need to be chained into the default calibration collection.
Second, calibrations generated in \texttt{main} need to be copied over to the \texttt{embargo} repo at USDF if they need to be applied to embargoed images.
...
\begin{tabular}{lll}
  ...
  \hline
  Summit & LATISS & LATISS \\
  Summit & LSSTComCam & LSSTComCam \\
  & & LSSTComCamSim \\
  Summit & LSSTCam & LSSTCam \\
  \hline
  Tucson Test Stand (TTS) & LATISS & LATISS \\
  Tucson Test Stand (TTS) & LSSTComCam & LSSTComCam \\
  & & LSSTComCamSim \\
  \hline
\end{tabular}
...
The way to copy calibration files from \texttt{main} to the Summit, BTS, and the other DFs is via \texttt{rucio}.
...
Currently, there is work being done to create automatic Rucio ``subscriptions'' such that the calibration group merely define a dataset and the automated system handles the distribution.
This depends on pre-determined naming conventions.
For calibrations to be distributed to the Summit and BTS, the \texttt{DATASET} name should be of the form \texttt{Dataset/\$INSTRUMENT/calib/\$TICKET}, matching the chained calibration collection name.
For calibrations that are not to be distributed to the Summit and BTS, and only the other DFs, the \texttt{DATASET} name should be of the form \texttt{Dataset/historical/\$INSTRUMENT/calib/\$TICKET}, matching the chained calibration collection name.
These are calibrations that are not to be used for online processing, but only for (e.g.) data releases where the calibrations only apply to older data.

The following is how a set of darks and flats can be synchronized.
...
DATASET=Dataset/$INSTRUMENT/calib/$TICKET
...
# looks as expected.
rucio did content list ancillary:$DATASET
...
rucio replica list dataset ancillary:$DATASET
...
butler import $REPO s3://butler@rubinobs-calibrations/rucio/ancillary \
       --export-file `pwd`/export.yaml -t direct
...
\begin{verbatim}
butler ingest-photodiode main lsst.obs.lsst.LsstCam \
...
\item{\texttt{export REPO=main}: The name of the butler repository to generate calibs.}
...
The \verb|cp_butler| package contains a set of utilities intended to make management of calibration products more convenient.
These utilities are not of general use, and so have not previously been available for the command line.
After setting up the package, added subcommands will become available for \verb|butler|, under the \verb|calibtool| name space.
This section will describe these added subcommands, with some details about how they function.
Note that subcommands that update the database require an explicit \verb|--dry-run False| argument.

% something something TAXICAB.

\subsection{probe}
The \verb|butler calibtool probe| subcommand queries the association table for calibration datasets, and returns a summary of that data.
Most routine uses of this subcommand will be used to identify the RUN collection associated with a certified calibration, and to retrieve the timespan for which the calibration is certified.

\subsection{decertify}
Certification errors have happened in the past, and although the butler API does support decertification, those errors have largely been dealt with by removing the incorrect CALIBRATION collection from the standard collection chains.
This subcommand provides command line access to that API, with the addition of a series of checks to help prevent further errors.
Only single CALIBRATION collections can be supplied, to prevent any unintended consequences when recursing through CHAINED collections.
RUN collections do not have the requisite association table information containing certified dates, so they are also not allowed.
The begin and end dates for the calibration to be decertified must be specified if they are defined (and not infinite), as these are used to select which datasets to decertify.

\subsection{recertify}
The butler API allows for the certification date range to be updated ``in place,'' without fully removing the certification.
The \verb|butler calibtool recertify| subcommand takes advantage of this method.
All of the same restrictions apply as with \verb|decertify|, with the added constraint that the updated validity date range must be a subset of the existing validity range.
Shifting a calibration by a fixed amount does not generally result in a subset of the existing validity, so calibrations that need this kind of change may need to be decertified and passed to `butler certify-calibrations` again.

\subsection{Examples}
The following block presents some examples of the use of these commands to update calibrations, using the commands that were used as part of DM-55577 and TAXICAB-68.
\begin{footnotesize}
\begin{verbatim}
# An example probe, truncated for simplicity.
# Executing this command will return all detectors and all calibration types.
butler calibtool probe $REPO --collections LSSTCam/calib/DM-53620
\end{verbatim}

\begin{table}
  \scriptsize
  \caption{Output of butler calibtool probe.  This table has been reformatted to fit this document.}
  \begin{tabular}{l|l}
    \hline
    calib\_type & gen\_run \\
    & calib\_collection \\
    & calib\_dataId\\
    & calib\_timespan \\
    \hline
    bias        &\verb|LSSTCam/calib/DM-53620/3s_v1_dp2_v2/biasGen.20251215c/20251218T023846Z|  \\
                &\verb|LSSTCam/calib/DM-53620/3s_v1_dp2_v2/bias.20251215c| \\
                & {instrument: 'LSSTCam', detector: 84} \\
                & [2025-07-03T00:00:00, ∞) \\
    \hline
    gain\_correction &\verb|LSSTCam/calib/DM-53620/3s_v1_dp2_v2_gain_correction_20250720/gainCorrectionGen.20251215a/20251218T183917Z| \\
                & \verb|LSSTCam/calib/DM-53620/3s_v1_dp2_v2_gain_correction_20250720/gainCorrection.20251215a| \\
                & {instrument: 'LSSTCam', detector: 84} \\
                & [2025-07-03T00:00:00, 2025-08-11T00:00:00)\\
    \hline
  \end{tabular}
\end{table}

\begin{verbatim}
# A simple recertification.
# As this is shifting a validity range with an infinite extent, the new validity range is a subset of the old.
butler calibtool recertify $REPO --collections LSSTCam/calib/DM-53620/3s_v1_dp2_v2/bias.20251215c \
       -d bias --begin-date 2025-07-03T00:00:00 --new-begin-date 2025-07-03T12:00:00 \
       --dry-run False

# A more complicated recertification requiring decertification.
# This is not a subset of the old range, and so needs a two-step process.
butler calibtool decertify $REPO
  --collections LSSTCam/calib/DM-53620/3s_v1_dp2_v2_gain_correction_20250720/gainCorrection.20251215a \
  -d gain_correction --begin-date 2025-07-03T00:00:00 --end-date 2025-08-11T00:00:00 --dry-run False

butler certify-calibrations $REPO --begin-date 2025-07-03T12:00:00 --end-date 2025-08-11T12:00:00 \
 LSSTCam/calib/DM-53620/3s_v1_dp2_v2_gain_correction_20250720/gainCorrectionGen.20251215a/20251218T183917Z \
 LSSTCam/calib/DM-53620/3s_v1_dp2_v2_gain_correction_20250720/gainCorrection.20251215a \
 gain_correction
\end{verbatim}
\end{footnotesize}
