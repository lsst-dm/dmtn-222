...
Note that the known failures where exported calibrations had incorrect permissions have all been fixed.
...
Once the rule is created then the \texttt{rucio} service will do the necessary synchronizations, and will place the calibrations in special S3 calibration buckets at BTS and the Summit and any other DFs that are configured.
